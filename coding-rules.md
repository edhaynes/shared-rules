...
15. Autonomy is bounded by version control. Claude sessions run unattended (autocomplete / `--dangerously-skip-permissions`) **only** when the working directory is an independent git repository under version control whose remote is synced to `github.com/edhaynes`. In that mode the LLM MAY read files anywhere on the machine, but MUST NOT write, create, modify, or delete anything outside a versioned, git-backed-up repository. The remote git history is the safety net that justifies skipping permission prompts — no recoverable history, no autonomy. If a task would require writing outside such a repo, the LLM MUST stop and ask.
